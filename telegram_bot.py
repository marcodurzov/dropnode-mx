# =============================================================
# DROPNODE MX — telegram_bot.py v2.5
# - Sin hora en mensajes
# - Stock: no muestra numero cuando es valor default
# - Hashtags al final de cada alerta VIP
# - Setup canal free con mensaje fijado corregido
# =============================================================
...
def link_auto(url, item_id=""):
    if "mercadolibre" in url:
        return link_ml(url, item_id)
    if "amazon.com.mx" in url or "amazon.mx" in url:
        return link_amazon(url)
    return url
...
def construir_contexto_historico(producto_id: str, precio_actual: float) -> dict:
...
def construir_prueba_social(producto_id: str, stock) -> dict:
    eng = get_engagement_producto(producto_id)
    ps  = {}
    # Solo mostrar stock si es valor real detectado (no el default 10 o None)
...
def estimar_ventana_oferta(descuento: float, stock) -> str:
    ...
    if descuento >= 0.50 or stk <= 2:
        return "Oferta podria terminar en minutos"
    elif descuento >= 0.35 or stk <= 5:
...
def formatear_free(alerta: dict) -> str:
    ...
    if ps.get("clicks"):
        msg += f"_{ps['clicks']}_\n"
    msg += f"\n[Ver oferta]({link})\n\n"
    msg += "_El canal VIP recibio esta alerta primero con analisis de reventa._\n"
...
def formatear_mejor_del_dia(alertas: list) -> str:
    ...
    for i, item in enumerate(alertas[:5], 1):
        ...
        ctx    = construir_contexto_historico(item.get("producto_id", ""), precio)
        ...
        msg += linea + "\n\n"
    msg += "🔒 _Alertas de errores de precio van al VIP primero._\n"
    msg += f"_{LAUNCHPASS_LINK}_"
    return msg
...
def formatear_recordatorio_vip():
    lnk = (
        f'<a href="{LAUNCHPASS_LINK}">Canal DropNode VIP</a>'
...
def mensaje_bienvenida_grupo():
    vip_link = (
        f'<a href="{LAUNCHPASS_LINK}">Canal VIP DropNode</a>'
...
def enviar_resumen_diario(total_vip=0, total_free=0):
    ...
    lnk   = (
        f'<a href="{LAUNCHPASS_LINK}">Acceso VIP</a>'
...
DOMINIOS_OK   = [
    "mercadolibre", "amazon", "liverpool", "walmart",
...
def canal_free_tiene_fijado():
    try:
        r = requests.get(
            ...
            params={"chat_id": CHANNEL_FREE_ID},
            timeout=10,
        )
...
def setup_canal_free():
    texto = (
        ...
        "Nuestro equipo revisa miles de productos todos los dias.\n"
        ...
        "Ofertas seleccionadas de las principales tiendas de Mexico\n"
        "Cupones activos combinados con precios bajos\n\n"
        "<b>Canal VIP ($299 MXN/mes):</b>\n"
        "Alertas en tiempo real — antes que este canal\n"
        "Productos y errores de precio que nunca llegan aqui\n"
        "Analisis de reventa incluido en cada alerta\n"
        "Acceso a oportunidades que el canal publico no ve\n\n"
        "<i>DropNode MX — Siempre encontramos el mejor precio.</i>"
    )
    payload = {
    ...
    try:
        ...
        if data.get("ok"):
            ...
            return True
        logger.error(f"[SETUP] Error: {data.get('description', '?')}")
        return False
    except Exception as e:
